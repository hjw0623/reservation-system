(function (){
	var getPlistUrl = '/bottom/category';
	var bottomUrl='/bottom';
	var num = 10;
	var categoryId =1; 
	
	// 카테고리 목록 조회, 테이블 스키마에서 1은 전체 카테고리. 
	function getProductListByCategory(getProductListUrl, categoryId, num){
		$.ajax({
			method:"GET",
	...
	//특정 카테고리 상품의 count조회 
	function getCountProductList(getProductListUrl, categoryId){
		$.ajax({
			method:"GET",
			url: getProductListUrl,
	...
	$('.event_tab_lst.tab_lst_min').on("click", ".item", function(event){
		num=10; //init
		$('.event_tab_lst.tab_lst_min .anchor').removeClass('active');
		$(this).find('.anchor').addClass('active');
		categoryId = $(this).data('category');	
		getProductListByCategory(getPlistUrl, categoryId, num); 
		getCountProductList(bottomUrl, categoryId);	
	}).bind(num).bind(categoryId);
	
	//더보기 버튼 이벤트 
	$('.more').on("click", ".btn", function(){
		num+=10;	
		getProductListByCategory(getPlistUrl, categoryId, num); 
	}).bind(num);
})();